...
export default function TourCardBooking() {
  ...
  const tour = {
    ...
    image:
      "https://cdn.vntrip.vn/cam-nang/wp-content/uploads/2020/06/tuong-phat-thich-ca-27m-tren-nui-linh-ung.jpg",
      ...
      {activeTab === "upcoming" ? (
        <div className="bg-white rounded-md shadow border border-gray-200 p-4 mb-6 bg-opacity-60">
          ...
          <div className="flex gap-6 items-center mb-4 border-b ">
            ...
            <p className="text-sm mb-2">
              Trạng thái:{" "}
              <span className={`${tour.status === "0" ? "text-blue-950" : tour.status === "1" ? "text-green-600" : tour.status === "2" ? "text-red-800" : ""} font-semibold`}>
                {tour.status === "0"
                  ? "Chờ thanh toán"
                  : tour.status === "1"
                  ? "Đã thanh toán"
          ...
          <div className="flex gap-4">
            ...
            <div className="flex items-start gap-3 text-gray-600 text-lg">
              ...
              <button title="Chỉnh sửa" className="hover:text-red-600">
                <BsPencil />
              </button>
              <button title="Khác" className="hover:text-red-600">
                <BsThreeDots />
              </button>
            </div>
          </div>
